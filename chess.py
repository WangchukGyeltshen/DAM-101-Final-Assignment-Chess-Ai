...
class Chess(object):
    ...
    def reset(self):
        ...
        # Always set white as the first player to move
        self.turn["black"] = 0
        self.turn["white"] = 1
        ...
        # Reset the board based on the player's color
        if hasattr(self, "player_color") and self.player_color == "black":
            ...
            # Default setup for white perspective
            for i in range(97, 105):
                for x in range(1, 9):
                    if x == 1:
                        if chr(i) == 'a' or chr(i) == 'h':
    ...
    def possible_moves(self, piece_name, piece_coord):
        ...
        if self.last_move:
            ...
            if last_piece[6:] == "pawn" and abs(last_start[1] - last_end[1]) == 2:
                # Check if the current pawn can capture en passant
                if piece_name[:5] == "white" and last_end[1] == piece_coord[1] - 1:
                    if last_end[0] == piece_coord[0] - 1 or last_end[0] == piece_coord[0] + 1:
                        positions.append([last_end[0], last_end[1] + 1])
                elif piece_name[:5] == "black" and last_end[1] == piece_coord[1] + 1:
                    if last_end[0] == piece_coord[0] - 1 or last_end[0] == piece_coord[0] + 1:
                        positions.append([last_end[0], last_end[1] - 1])
    ...
    def validate_move(self, destination):
        ...
        if self.last_move:
            last_piece, last_start, last_end = self.last_move
            if last_piece[6:] == "pawn" and abs(last_start[1] - last_end[1]) == 2:
                if abs(destination[0] - last_end[0]) == 0 and abs(destination[1] - last_end[1]) == 1:
                    # Remove the captured pawn
                    captured_col = chr(97 + last_end[0])
                    captured_row = 8 - last_end[1]
                    self.piece_location[captured_col][captured_row][0] = ""

        # Update the last move
        self.last_move = (src_name, [ord(k) - 97, 8 - key], destination)
